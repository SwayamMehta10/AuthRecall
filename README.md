# AuthRecall - OAuth Account Tracker

Never forget which Google account you used to sign in to your favorite websites. AuthRecall automatically tracks and remembers your OAuth authentication history.

https://github.com/user-attachments/assets/6896cf5e-5fa4-49b5-8264-5aa5c4f39adb

## 🚀 Download

***Launching Soon on the Chrome Web Store!***

## ✨ Features

### **Automatic Detection**

AuthRecall intelligently monitors your Google OAuth logins and automatically remembers which email you used for each website. No manual tracking required.

### **Visual Badge Indicator**

See at a glance which account you used on any site with the extension badge that displays your email directly in the toolbar.

### **Comprehensive Dashboard**

View all your tracked accounts and websites in an organized, searchable popup interface. Quickly find which account goes with which service.

### **Manual Entry Support**

Forgot to install AuthRecall before logging in? No problem! Add accounts manually for any website.

### **Privacy First**

-   All data is stored **locally** in your browser
-   No data is sent to external servers
-   You own and control your information
-   Optional Notion sync (requires your own API key)

### **Import & Export**

Easily backup your account mappings or transfer them between browsers with one-click import/export functionality.

### **Notion Integration** (Optional)

Sync your account data to Notion for backup or cross-browser access. You maintain full control with your own API key.

### **Smart Notifications**

Get notified when AuthRecall detects a new Google login, keeping you informed of your authentication activity.

## 📖 How It Works

1. **Install the extension** from the Chrome Web Store
2. **Log in with Google** on any website
3. **AuthRecall automatically saves** which email you used
4. **Check the badge** next time you visit to see which account to use

AuthRecall uses a universal scanner that monitors network responses for email patterns and detects Google OAuth authentication flows automatically.

## 🛠️ Manual Installation (For Developers)

1. Clone this repository
2. Open `chrome://extensions` in Chrome
3. Enable "Developer mode" (toggle in top right)
4. Click "Load unpacked"
5. Select the AuthRecall folder

## 💡 Use Cases

-   **Multiple work accounts**: Remember which work email you used for each SaaS tool
-   **Personal vs. professional**: Keep track of which account type you used where
-   **Side projects**: Manage different accounts for different projects
-   **Shared computers**: Quickly identify which account you should use

## 🙏 Support

If you find AuthRecall helpful, please consider:

-   Starring this repository
-   Leaving a review on the Chrome Web Store
-   Reporting bugs or suggesting features via GitHub Issues
-   Supporting the development (contributions welcome!)

Your feedback helps make AuthRecall better for everyone!
